...
function calculateMetrics(insight) {
  ...
  let engagements = 0, shares = 0, comments = 0, videoViews = 0, follows = 0, reactions = 0;
  for (const action of (insight.actions || [])) {
    ...
    if (action.action_type === 'like') follows += parseInt(action.value);
    if (action.action_type === 'page_engagement') follows += parseInt(action.value);
    if (action.action_type === 'post_reaction') reactions = parseInt(action.value);
  }
  return {
    campaign_name: insight.campaign_name,
    campaign_id: insight.campaign_id,
    spend, impressions, reach, clicks, engagements, shares, comments,
    video_views: videoViews, follows, reactions,
    cpm: impressions > 0 ? Math.round(spend / impressions * 1000 * 100) / 100 : 0,
    eng_per_peso: spend > 0 ? Math.round(engagements / spend * 100) / 100 : 0,
    views_per_peso: spend > 0 ? Math.round(videoViews / spend * 100) / 100 : 0,
