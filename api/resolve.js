...
// ─── Meta (FB/IG) resolution handler ───
async function resolveMeta(url, res) {
...
// ─── Main handler: auto-detect YouTube vs Meta ───
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (isYouTube) {
      return await resolveYouTube(url, res);
    } else {
      return await resolveMeta(url, res);
    }
  } catch (err) {
